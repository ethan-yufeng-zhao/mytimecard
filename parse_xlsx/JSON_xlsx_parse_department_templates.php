<?php
    // parse_xlsx/JSON_xlsx_parse_department_templates.php
    ...
    foreach ($json_all_certs as $cert_id => $value) {
    ...
    foreach ($json_all_templates as $template_id => $value) {
        if($value['template_is_default_for_department'] == 1) {
            $dept_arr[$value['template_department_number']] = $template_id;
        }
    }
    ...
    $arr['template_check_problem_list']['dept'] = array();

    $running_row = 2;

    while(strlen(trim($objPHPExcel->getActiveSheet()->getCell('D'.$running_row))) > 0) { //Row must be populated
        $temp_arr = array();
        $temp_arr['cert_name'] = trim($objPHPExcel->getActiveSheet()->getCell('D'.$running_row));
        $temp_arr['template_department_number'] = trim($objPHPExcel->getActiveSheet()->getCell('A'.$running_row));
        ...
        if (array_key_exists($temp_arr['template_department_number'], $dept_arr)) {
            $temp_arr['template_id'] = $dept_arr[$temp_arr['template_department_number']];
        } else {
            $arr['template_check_problem_list']['dept'][] = 'Dept ' . $temp_arr['template_department_number'] . 'Not Found';
